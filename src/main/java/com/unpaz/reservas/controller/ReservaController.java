...
public class ReservaController implements GenericResponseController {
    ...
    public ResponseEntity<?> getAll(@Valid @RequestBody DtoReqFecha fechaDto)throws NotFoundException, BadRequestException {
        clearResponse();
        response.put("Lista de horarios libres : ", reservaServiceImp.listarHorasLibres(fechaDto.getFecha()));
        return getResponse(HttpStatus.OK);
    }

    //este nos crea una reserva
    @PostMapping("/reservation")
    public ResponseEntity<?> createReserva(@Valid @RequestBody ReservaDtoReq reserva)throws NotFoundException, BadRequestException {
        clearResponse();
    ...
    public ResponseEntity<Map<String, Object>> modificarReserva(@RequestBody ReservaDto obj)throws NotFoundException, BadRequestException{
        clearResponse();
